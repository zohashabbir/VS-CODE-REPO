...
class VoiceChatSessionControllerFactory {
	...
	static async create(accessor: ServicesAccessor, context: 'inline' | 'terminal' | 'quick' | 'view' | 'focused'): Promise<IVoiceChatSessionController | undefined> {
		...
		if (context === 'focused') {
			// This needs to be first because the chat input can be focused
			// while the terminal chat is active, #207362
			const activeInstance = terminalService.activeInstance;
			if (activeInstance) {
				const terminalChat = TerminalChatController.activeChatWidget;
