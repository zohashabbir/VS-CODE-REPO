...
import {
	...
	KeyedTaskIdentifier as NKeyedTaskIdentifier, TaskDefinition, RuntimeType, InstancePolicy
...
export abstract class AbstractTaskService extends Disposable implements ITaskService {
	...
	private handleInstancePolicy(task: Task, policy?: InstancePolicy): void {
		switch (policy) {
			case InstancePolicy.terminateNewest:
				this.restart(this.getTaskSystem().getLastInstance(task) ?? task);
				break;
			case InstancePolicy.terminateOldest:
				this.restart(this.getTaskSystem().getFirstInstance(task) ?? task);
				break;
	...
	private async createTaskQuickPickEntries(tasks: Task[], group: boolean = false, sort: boolean = false, selectedEntry?: TaskQuickPickEntry, includeRecents: boolean = true): Promise<TaskQuickPickEntry[]> {
		...
		const TaskQuickPickEntry = (task: Task): TaskQuickPickEntry => {
			...
			const commonKey = task._id.split('|')[0];
			if (count[commonKey]) {
